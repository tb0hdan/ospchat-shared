...
/**
 * Desktop (JVM) actual of [SigningCrypto]. Uses Bouncy Castle's lightweight
 * Ed25519 API (zero JCE provider registration required). Mirrored
 * byte-for-byte in `androidMain` — both targets are JVM-based and ship the
 * same `bcprov-jdk18on` artifact, but Kotlin Multiplatform 2.0.21 in this
 * project doesn't share a `jvmMain` intermediate sourceset, so the impls
 * are duplicated rather than hoisted.
 */
